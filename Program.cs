...
namespace MachineLearningPlatformer
{
    ...
    class Graphics //   Class to contain all methods related to on-screen display.
    {
        ...
        public static void DrawInfo()
        {
            ...
            */
            ...
            try
            {
                Console.Write(" Fit.: {1}, Frm.: {0}  ", GameVars.CurrentFrame, Player.Fitness);
            }
            ...
            try
            {
                Console.Write(" HiFit.: {2}, Gen.: {1}, Inp.: {0}    ", Player.NextDirection, Machine.Generation, Machine.HighestFitness);
            }
            catch { }; // Catches any exceptions, in the case that the string to write exceeds the width of the console - prevents crashing.
    ...
    class Machine
    {
        ...
        public static void CaptureArea()
        {
            ...
            for(int y = Player.Y_Pos - 1; y <= Player.Y_Pos + 1; y++)
            {
                for(int x = Player.X_Pos - 1; x <= Player.X_Pos; x++)
                {
